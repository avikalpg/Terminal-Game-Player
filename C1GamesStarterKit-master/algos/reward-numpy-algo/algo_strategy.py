...
class AlgoStrategy(gamelib.AlgoCore):
    ...
    def build_firewall(self, game_state, firewall_strategy):
        for item in firewall_strategy:
            if not game_state.contains_stationary_unit(item['location']):
                if game_state.can_spawn(item['type'], item['location']):
                    game_state.attempt_spawn(item['type'], item['location'])
                else:
                    gamelib.debug_write(">> Running out of CORES (trying to save for future moves)")
                    break

    def deploy_information(self, game_state, information_strategy):
       for item in information_strategy:
            if not game_state.contains_stationary_unit(item['location']):
                if game_state.can_spawn(item['type'], item['location'], item['intensity']):
                    game_state.attempt_spawn(item['type'], item['location'], item['intensity'])
                else:
                    i = 0
                    while (i < item['intensity']) and game_state.can_spawn(item['type'], item['location']):
                        game_state.attempt_spawn(item['type'], item['location'])
                    gamelib.debug_write(">> Running out of BITS (trying to save for future moves)")
                    break
