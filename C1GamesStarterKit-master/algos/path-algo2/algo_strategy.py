...
class AlgoStrategy(gamelib.AlgoCore):
    ...
    def build_defences(self, game_state):
        ...
        are more effective.
        """
        firewall_locations = [[25, 11], [22, 12], [23, 9], [24, 13], [27, 13]]
        for location in firewall_locations:
            if game_state.can_spawn(ENCRYPTOR, location):
                game_state.attempt_spawn(ENCRYPTOR, location)
        ...
        firewall_locations = [[16, 12], [17, 12], [18, 12], [19, 12]]
        for location in firewall_locations:
            if game_state.can_spawn(ENCRYPTOR, location):
                game_state.attempt_spawn(ENCRYPTOR, location)
    ...
    def deploy_attackers(self, game_state):
        ...
        if game_state.turn_number > 25:
            if game_state.turn_number % 10 == 0:
                if game_state.can_spawn(EMP, [26, 12], 1):
                    game_state.attempt_spawn(EMP, [26, 12], 1)
            elif game_state.turn_number % 10 == 5:
                if game_state.can_spawn(DESTRUCTOR, [14, 0], 1):
                    game_state.attempt_spawn(DESTRUCTOR, [14, 0], 1)
            if game_state.can_spawn(PING, [13, 0], game_state.turn_number//4):
                game_state.attempt_spawn(PING, [13, 0], game_state.turn_number//4)
            else:
            ...
            if game_state.turn_number % 4 == 0:
                ...
                if game_state.can_spawn(EMP, [26, 12], 2):
                    game_state.attempt_spawn(EMP, [26, 12], 2)
