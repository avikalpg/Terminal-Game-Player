...
class AlgoStrategy(gamelib.AlgoCore):
    ...
    def on_turn(self, turn_state):
        ...
        turn_state = json.loads(turn_state)
        if game_state.turn_number == 0:
            self.starter_strategy(game_state)
        else:
            self.subsequent_strategy(game_state)
        # sys.stderr.write("=================== TURN STATE ====================\n")
        # sys.stderr.write("P1stats 3 = " + str(turn_state['p1Stats'][3]) + "P2stats 3 = " + str(turn_state['p2Stats'][3]))
        # sys.stderr.write(str(turn_state) + "\n\n")
        # sys.stderr.write("=================== CONFIG ====================\n")
        # sys.stderr.write(str(self.config) + "\n\n")
        # sys.stderr.write("=================== GAME STATE ====================\n")
    ...
    def build_defences(self, game_state):
        ...
        """
        firewall_locations = [[1, 12], [26, 12], [7, 11], [14, 11], [21, 11], [15,6]]
        for location in firewall_locations:
            if game_state.can_spawn(DESTRUCTOR, location):
                game_state.attempt_spawn(DESTRUCTOR, location)

        firewall_locations = [[0, 13], [27, 13], [7, 12], [14, 12], [21, 12], [15,7]]
        for location in firewall_locations:
    ...
    def subsequent_strategy(self, game_state):
        ...
        md = framework.FrameWork(game_state)
        strategy = md.play()
        """
        Then build defenses.
        """
        self.build_firewall(game_state, strategy['firewall'])

        """
        Finally deploy our information units to attack.
        """
        self.deploy_information(game_state, strategy['information'])

    def build_firewall(self, game_state, firewall_strategy):
        firewall_locations = firewall_strategy['DF']
        for location in firewall_locations:
            if game_state.can_spawn(DESTRUCTOR, location):
                game_state.attempt_spawn(DESTRUCTOR, location)

        firewall_locations = firewall_strategy['FF']
        for location in firewall_locations:
            if game_state.can_spawn(FILTER, location):
                game_state.attempt_spawn(FILTER, location)

        firewall_locations = firewall_strategy['EF']
        for location in firewall_locations:
            if game_state.can_spawn(ENCRYPTOR, location):
                game_state.attempt_spawn(ENCRYPTOR, location)

    def deploy_information(self, game_state, information_strategy):
       
        # information_locations = information_strategy['EI']
        # for location, number in information_locations:
        #     if game_state.can_spawn(EMP, location, number):
        #         game_state.attempt_spawn(EMP, location, number)

        # information_locations = information_strategy['PI']
        # for location, number in information_locations:
        #     if game_state.can_spawn(PING, location, number):
        #         game_state.attempt_spawn(PING, location, number)

        # information_locations = information_strategy['SI']
        # for location, number in information_locations:
        #     if game_state.can_spawn(SCRAMBLER, location, number):            
        #         game_state.attempt_spawn(SCRAMBLER, location, number)
        
        information_locations = information_strategy['EI']
        for location, number in information_locations:
            for _ in range(0, number):
                sys.stderr.write("<=============== DROP EMP =================>")
                if game_state.can_spawn(EMP, location):
                    game_state.attempt_spawn(EMP, location)
