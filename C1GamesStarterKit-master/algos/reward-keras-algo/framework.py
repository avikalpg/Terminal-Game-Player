...
class FrameWork():
    ...
    Attributes:
        ...
        pi_locations: spawn locations for Pings
        ei_locations: spawn locations for EMPs
        si_locations: spawn locations for Scramblers
        ff_locations: firewall locations for Filters
        ef_locations: firewall locations for Encryptors
        df_locations: firewall locations for Destructors
    """
    ...
    def load_model_vars(self):
        ...
        try:
            with open(self.model_parameters_file, 'r') as f:
                self.model_parameters = f.read()
        except FileNotFoundError:
            self.model_parameters = self.initialize_model_parameters(self.model_parameters_file)

        try:
            with open(self.model_weights_file, 'r') as f:
                self.model_weights = f.read()
        except FileNotFoundError:
            self.model_weights = self.initialize_model_weights(self.model_weights_file)
        ...
        try:
            ...
            self.prev_game_state = self.initialize_prev_game_state(self.prev_game_state_file)
    ...
    def save_state(self):
        ...
        with open(self.prev_game_state_file, 'w+') as f:
            storage_game_state = self.create_storage_game_state(self.game_state)
            f.write(json.dumps(storage_game_state))

    def initialize_model_parameters(self, file_name):
        value = ""
        with open(file_name, 'w+') as f:
            f.write(value)
        return value

    def initialize_model_weights(self, file_name):
        value = ""
        with open(file_name, 'w+') as f:
            f.write(value)
        return value

    def initialize_prev_game_state(self, file_name):
        all_locations = []
        features = []
    ...
    def decide_information(self, friendly_locations):
        ...
        # numPings = random.randint(0, 5)
        # for _ in range(0, numPings):
        #     information_df.loc[information_df.index == random.randint(0, len(friendly_locations) - 1), "PI"] += 1
        # numEMPs = random.randint(0, 5)
        # for _ in range(0, numEMPs):
        #     information_df.loc[information_df.index == random.randint(0, len(friendly_locations) - 1), "EI"] += 1
        # numScramblers = random.randint(0, 3)
        # for _ in range(0, numScramblers):
        #     information_df.loc[information_df.index == random.randint(0, len(friendly_locations) - 1), "SI"] += 1

        information_df.loc[information_df.index == random.randint(0, len(friendly_locations) - 1), "EI"] += random.randint(1, 5)
        information_df.loc[information_df.index == random.randint(0, len(friendly_locations) - 1), "PI"] += random.randint(0, 5)
